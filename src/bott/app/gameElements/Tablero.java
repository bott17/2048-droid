...
/**
 * Representa el tablero de juego. Consta de un tablero de 4x4 casillas Se trata
 * de un objeto unico en todo el juego.
 * 
 * @author bott
 */
...
public class Tablero {
	...
	/**
	 * Constructor publico del tablero. Solo permite crear uno para todo el
	 * juego.
	 */
	...
	private boolean rellenaCasilla(Celda celda, int posicion) {
		...
		if (positionIsEmpty(posicion)) {
			...
			if (indiceCasillasLibres.contains(posicion)) {
				indiceCasillasLibres.remove(indiceCasillasLibres
						.indexOf(posicion));
			}
			return true;
		} else
	 ...
	 * @param aleatorio
	 *            Indica si la casilla puede contener un 4 o un 2 (true) o solo
	 *            un 2 (false)
	 */
	...
	/**
	 * Convierte un formato de filas y columnas basado en celda 11 a una key de
	 * identificacion dentro del mapa
	 * 
	 * @param fil
	 *            Fila donde se encuentra la celda, 0-13
	 * @param col
	 *            Columna donde se encuentra la celda, 0-13
	 * @return
	 */
	 ...
	 * 
	 * @exception Las
	 *                posiciones deben ser correctas
	 * @param posInicial
	 *            Donde se encuentra la celda a mover
	 * @param posFinal
	 *            Donde se va a colocar
	 * @param celda
	 *            Celda que se desea mover
	 */
	...
	private boolean sumaCelda(int posinicial, int posFinal) {
		...
		if (getCasilla(posinicial).getValor() == getCasilla(posFinal)
				...
				&& (getCasilla(posinicial).getTurnoSuma() < Game.getTurno())) {

//			Log.i(TAG,
//					"Casilla final: " + getCasilla(posFinal).getValor()
